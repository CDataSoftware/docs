...
When creating tables for [API Connector](/ja/APIConnector), it is important to remember that each table must be created individually in order to be listed in the Connector’s catalog. To do so, follow the steps below:
...
Enter the URL of your API endpoint into the **Request URL** field.
	<Note>
	**Note**: To connect to an API that requires a different request for each record, include the name of the column in curly braces. For example, `https://api.example.com/table/{id}`.
	</Note>
</Step>
<Step>
