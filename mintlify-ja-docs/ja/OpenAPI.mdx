...
<Warning>
**Important:** If a table’s schema within a workspace is modified (columns added or removed), you need to regenerate the OpenAPI specification file for the table’s workspace and re-import it into the client application.
</Warning>
## API Operations
...
- `$filter` applies OData operators, such as `gt` (greater than) or `eq` (equals) to the entities in the data set. See [$filter](/ja/OData-Query-Options#filter) for a complete list of operators.
