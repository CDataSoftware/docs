...
- The `snippets` directory contains reusable snippets for the documentation. When I started working on this, the `snippets` directory had to be at the top level. This requirement has since changed, but I will leave as is for now. This directory contains an `en` and `ja` folder. There is a `Data-Sources` subdirectory for each language. This contains the data source snippets that are referenced by the Connect AI and Embedded docs. There are other snippets for text that is repeated.
