...
<Steps>
<Step>
In the **Workspaces** screen in Connect AI, select a workspace and download the OpenAPI specification for the workspace. See [OpenAPI](/en/OpenAPI) for more information.
<Note>
...
![](./images/sfl_externaloperations.png)
	...
	The input parameters are appended to the URL, such as `$top`, `$filter`, and `$orderby`. See [API Operations](/en/OpenAPI#api-operations) for a complete parameter reference. The input parameters are based on OData filtering.
