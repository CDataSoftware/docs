...
description: "Embedded Cloud provides a full-featured API that allows any REST-compatible application or integration tool capable of creating HTTP requests to query data, perform batch query operations, and execute stored procedures across all data sources configured in your account. This API is based on a JSON-formatted API. The Connect AI API’s base URL is `https://cloud.cdata.com/api`."
---

## Authentication

All requests to the Embedded Cloud API must be properly authenticated using JSON Web Tokens (JWT). See [Authentication](/en/API/Authentication-Embedded) for details.
