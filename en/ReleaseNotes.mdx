...
- Fixed the issue when SCALE for VARCHAR was null, the Data Explorer returned an error message. 
...
- In the Excel Add-On, the OR operator is now supported within the Query Builder.
- In the Google Add-on, fixed an issue where Google rejected a user with a missing last name.
...
- SageIntacct (Update): Driver updated to the latest version. Also added support for OneLogin authentication. 
