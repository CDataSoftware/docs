...
In general, performance is best in [SuiteAnalytics](/en/NetSuiteSuiteAnalytics). Connect AI uses NetSuite's native SuiteAnalytics driver. We recommend using it if you have access to it. Note that SuiteAnalytics is not supported by the NetSuite drivers.

### Record Limits (SuiteQL)

- SuiteQL has a hard limit of 100,000 records, imposed by NetSuite.
- You can bypass the limit if the account has SuiteAnalytics Connect or if the schema is set to SuiteTalk rather than SuiteQL. If you have SuiteAnalytics Connect, make sure to set the **AccountsSupportsSuiteAnalyticsConnect** property to **True**.
...
View the complete list of permissions required by our driver [here](https://cdn.cdata.com/help/DNM/powerbi/pg_permissions.htm).
...
If you receive this error, you have not set the required **Custom Fields** permissions in NetSuite. See [Permissions Configurations](https://cdn.cdata.com/help/DNM/powerbi/pg_permissions.htm).
</Accordion>

<Accordion title="Too many requests (HTTP 429)">

You have exceeded the NetSuite concurrency limits. You can try adjusting the `RetryWaitTime` or `MaximumRequestRetries`.
</Accordion>

### SuiteAnalytics Errors
