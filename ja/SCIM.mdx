...
To enable SSO for your account, contact [CData Support](https://jp.cdata.com/support/).
...
To provision a new user, `POST` to your unique SCIM endpoint. The system provisions the user in Connect AI. There is no need for the user to verify via email. The new user has a default role of Query user. See [Users](/ja/Users) for details on user types. The new user only has access to connections that have user-defined credentials turned on.
...
The Connect AI [Audit Log](/ja/Logs#audit-log) displays SCIM user provision, update, and deletion. 
