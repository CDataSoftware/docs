...
- Search for specific data in your data sources using the SQL Editor or the [Query Builder](/ja/QueryBuilder).
...
<Note>
  For instructions on using the Query Builder to build queries, refer to [Query Builder](/ja/QueryBuilder).
</Note>
...
Select a connection. All available items appear in the left column.
	...
	- The blue double document icon to the right of the item indicates that the item is being cached. See [Caching](/ja/Caching) for more information.
</Step>
<Step>
...
The *derived views* feature of Data Explorer allows you to create and save queries that return dynamically populated data results from your connections. You can query your saved derived views from [Integrations](/ja/Integrations), and the results automatically update when the data in your connections change.
...
<Step>
You then enter the details about the scheduled query, such as how often it runs. See [Scheduled Queries](/ja/Scheduled-Queries) for details.
</Step>
</Steps>