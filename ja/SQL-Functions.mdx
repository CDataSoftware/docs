...
### [STRING Functions](/ja/STRING-Functions)

String functions perform string manipulations and return a string value. See [STRING Functions](/ja/STRING-Functions) for more details.

```bash
SELECT CONCAT(firstname, space(4), lastname) FROM Account WHERE Industry = 'Floppy Disks'
```

### [DATE Functions](/ja/DATE-Functions)

These functions perform date and date time manipulations. See [DATE Functions](/ja/DATE-Functions) for more details.

```bash
SELECT CURRENT_TIMESTAMP() FROM Account
```

### [Math Functions](/ja/MATH-Functions)

These functions provide mathematical operations. See [MATH Functions](/ja/MATH-Functions) for more details.
...
Aggregate functions return data from across multiple rows. See [Aggregate Functions](/ja/Aggregate-Functions) for more details.
