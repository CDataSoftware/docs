...
<Frame>
![](/ja/images/powered_by_cdata_accounts.png)
</Frame>
...
<Frame>
![](/ja/images/powered_by_cdata_users.png)
</Frame>
<Note>
...
<Steps>
<Step>
The administrator of the Embedded Cloud parent account [creates a JSON Web Token (JWT)](/ja/API/Authentication-Embedded), which consists of a private key and a corresponding public key. The private key must be stored according to the security requirements of the product integrated with Embedded Cloud.
</Step>
<Step>
...
Use [Create Account](/ja/API/Account-API#create-account) in the Embedded Cloud API to create a child account. You must supply the JWT that you created in [Create a Parent Account](#step-1:-create-a-parent-account).
...
In addition, Embedded Cloud provides a full-featured [REST API](/ja/API/REST-API). You can query data directly with any REST-compatible application or integration tool capable of creating HTTP requests.

### View Logs

Use the [Log API](/ja/API/Log-API) to view and download a list of logs for a Embedded Cloud account.