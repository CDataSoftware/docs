...
description: "Embedded Cloud supports connectivity to the Model Context Protocol (MCP), the open-source standard for connecting AI applications to external systems. It was developed by Anthropic to bridge the gap between structured data and large language models (LLMs). MCP can be used from any client integration that supports it, such as [Claude Code](/ClaudeCode-Client-Embedded), [Microsoft Copilot Studio](/MicrosoftCopilot-Client-Embedded), and [Gemini](/Gemini-Client-Embedded). See [AI Tools](/AITools-Embedded) for a complete list of AI integrations for Embedded Cloud."
---
...
Embedded Cloud supports OAuth JWT bearer token authentication. See [Authentication](/Authentication-Embedded) for details.
